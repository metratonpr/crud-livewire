    ...
    <x-jet-form-section submit="store">
        ...
        <x-slot name="form">
            <div class="col-span-6 sm:col-span-4">
                ...
                <x-jet-input id="newContact.name" type="text" class="mt-1 block w-full"
                    wire:model.defer="newContact.name" autofocus />
                <x-jet-input-error for="newContact.name" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="newContact.email" value="{{__('E-mail')}}" />
                <x-jet-input id="newContact.email" type="text" class="mt-1 block w-full"
                    wire:model.defer="newContact.email" autofocus />
                <x-jet-input-error for="newContact.email" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="newContact.phone" value="{{__('Phone')}}" />
                <x-jet-input id="newContact.phone" type="text" class="mt-1 block w-full"
                    wire:model.defer="newContact.phone" autofocus />
                <x-jet-input-error for="newContact.phone" class="mt-2" />
            </div>
            <div class="col-span-6 sm:col-span-4">
                <x-jet-label for="newContact.message" value="{{__('Message')}}" />
                <textarea id="newContact.message" class="mt-1 block w-full" wire:model.defer="newContact.message"
                    autofocus>
                </textarea>
